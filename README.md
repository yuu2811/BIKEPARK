...
| # | ステップ | ステータス | 備考 |
|---|---------|-----------|------|
| 1 | リポジトリ作成 & `npm install` | ✅ 完了 | |
| 2 | Supabase プロジェクト作成 | ✅ 完了 | `bwgzkemhkihfouewujtp` |
| 3 | DB マイグレーション実行 | ⬜ 未実施 | SQL Editor で 3 ファイル実行が必要 |
| 4 | Storage バケット作成 | ⬜ 未実施 | `spot-photos`, `review-photos` |
| 5 | Google Maps API 有効化 & キー発行 | ✅ 完了 | Maps JavaScript / Places / Directions / Geocoding |
| 6 | Google Map ID 作成 | ✅ 完了 | |
| 7 | Google OAuth クライアント ID 作成 | ✅ 完了 | Google Cloud Console にて作成済み |
| 8 | Google OAuth **クライアントシークレット** を Supabase に設定 | ❌ **未完了** | PC ブラウザから GCP Console で確認 → Supabase Auth に設定が必要 |
| 9 | `.env.local` 設定 | ✅ 完了 | Supabase URL/Key, Maps API Key/Map ID |
| 10 | 開発サーバー動作確認 | ⬜ 未実施 | OAuth 設定完了後に確認 |

### 残課題

- **Google OAuth クライアントシークレットの設定** — Google Cloud Console の「認証情報」→ OAuth 2.0 クライアント ID → BIKEPARK をクリック → クライアントシークレットをコピー → Supabase Dashboard の **Authentication > Providers > Google** に Client ID と Client Secret を設定。スマホでは表示が難しいため **PC ブラウザ推奨**。JSON ファイルのダウンロードでも取得可。
- **DB マイグレーション** — Supabase SQL Editor で 3 ファイル実行
...
2. 作成後、**クライアント ID** と **クライアントシークレット** を控える
   - シークレットが画面に表示されない場合: 認証情報一覧から該当クライアント ID をクリックして詳細画面で確認、または JSON ダウンロードで取得
3. Supabase Dashboard の **Authentication > Providers > Google** を有効化:
...
`.env.local` を作成 (`.env.example` をコピーして値を設定):
